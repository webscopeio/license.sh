...
class LicenseValidationsTestCase(unittest.TestCase):
  ...
  def test_simple_validation_test(self):
    ...
    self.assertTrue(is_license_ok('MIT', whitelist), "MIT should not be not problematic, it's in a whitelist")
  ...
  def test_complex_or_license_expression(self):
    whitelist = ['MIT', 'Apache-3.0']
    self.assertTrue(is_license_ok('((MIT and Apache-3.0) OR Apache-2.0)', whitelist))
    self.assertTrue(is_license_ok('((License-2 or Apache-3.0) OR Apache-2.0)', whitelist))
    self.assertFalse(is_license_ok('((License-2 and Apache-3.0) OR Apache-2.0)', whitelist))

  def test_nested_invalid_expression_inside_complex_or_license_expression(self):
    whitelist = ['MIT', 'Apache-3.0']
    self.assertFalse(is_license_ok('((License-2, Apache-3.0) OR Apache-2.0)', whitelist))

  def test_multi_operator_expression(self):
    whitelist = ['MIT']
    self.assertEqual(is_license_ok('(BSD-2-Clause OR MIT OR Apache-2.0)', whitelist), True)
